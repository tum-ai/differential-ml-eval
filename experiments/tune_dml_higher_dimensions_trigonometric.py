...
def build_objective(vanilla_network: bool, data_generator: Callable, n_datapoints: int):
    def objective(trial: Trial):
        ...
        n_epochs = 100  # trial.suggest_categorical('epochs', [200, 500, 1000])
        activation = trial.suggest_categorical('activation', ['relu', 'sigmoid'])
        regularization_scale = 0  # trial.suggest_float('regularization_scale', 0.00001, 1, log=True)
        test_loss = train(
...
def main(vanilla_network: bool, dimensions: int, n_datapoints: int):
    ...
    study = optuna.create_study(
        ...
        study_name=f"lbfgs-trigonometric-data={n_datapoints}-dimensions={dimensions}-{vanilla_indicator}",
        direction="minimize",
        load_if_exists=True,
    )
...
if __name__ == "__main__":
    ...
    # main(True, 2, 2000)
    t1 = threading.Thread(target=main, args=(True, 2, 512))
    t2 = threading.Thread(target=main, args=(False, 2, 512))
    t1.start()
    t2.start()
